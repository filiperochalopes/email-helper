...
def test_stack_is_up_true(monkeypatch):
    ...
    monkeypatch.setattr(runner.subprocess, "run",
                        lambda *a, **k: SimpleNamespace(stdout="db\napp\nredis\n"))
    assert runner.stack_is_up("app") is True


def test_stack_is_up_false_without_docker(monkeypatch):
    monkeypatch.setattr(runner.shutil, "which", lambda _: None)
    assert runner.stack_is_up("app") is False


def test_run_in_stack_uses_docker_when_up(monkeypatch):
    monkeypatch.setattr(runner, "stack_is_up", lambda service="app": True)
    ...
    assert fake.cmd[:5] == ["docker", "compose", "exec", "-T", "app"]


def test_run_in_stack_falls_back_to_host(monkeypatch):
    monkeypatch.setattr(runner, "stack_is_up", lambda service="app": False)
    monkeypatch.setattr(runner.shutil, "which", lambda _: "/venv/bin/email-agent")
    fake = _fake_run(stdout="ok")
    monkeypatch.setattr(runner.subprocess, "run", fake)
    res = runner.run_in_stack(["accounts", "list"])
    assert res.where == "host"
    assert fake.cmd[0] == "/venv/bin/email-agent"
...
def test_run_handles_timeout(monkeypatch):
    ...
    monkeypatch.setattr(runner, "stack_is_up", lambda service="app": False)
    monkeypatch.setattr(runner.shutil, "which", lambda _: "email-agent")
